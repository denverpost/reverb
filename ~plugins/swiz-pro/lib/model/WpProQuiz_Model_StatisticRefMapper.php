...
// 		}
        ...
        if($this->checkPhotoOrGiveAwayContestQuizStats($statisticRefModel->getQuizId())){
            $values[] = '( ' . implode(', ', array(
                    'statistic_ref_id' => $refId,
                    'question_id' => 0,
                    'correct_count' => 0,
     ...
     * @return WpProQuiz_Model_StatisticHistory[]
     */
    public function fetchPcImages($quizId, $fieldId, $start, $limit)
    {
        $where = ' AND sf.form_data LIKE \'%"'.$fieldId.'":"1"%\'';
        ...
        $totalPcRows = $this->_wpdb->get_row("SELECT FOUND_ROWS() as countAll",ARRAY_A);
        ...
        return array('results' => $r,'totalPcRows' => $totalPcRows['countAll']);
    }
    ...
    public function approveByRefId($refId,$quizId)
    {
        $formMapper=new WpProQuiz_Model_FormMapper();

        $field_id=$formMapper->ApproveFieldId($quizId);

        $form_data_results=$this->_wpdb->get_var(
            $this->_wpdb->prepare(
                "SELECT form_data FROM {$this->_tableStatisticRef} WHERE statistic_ref_id = %d",
                $refId
            )
        );

        $form_data=json_decode($form_data_results, true);

        if("0" == $form_data[$field_id] || !isset($form_data[$field_id])) {
            $form_data[$field_id]="1";
        } else {
            $form_data[$field_id]="0";
        }

        $updated_form_data=json_encode($form_data);

        return $this->_wpdb->update(
        $this->_tableStatisticRef,
            array('form_data' => $updated_form_data),
            array( 'statistic_ref_id' =>$refId ),
            array('%s'),
            array( '%d' )
        );
    }
