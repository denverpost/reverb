...
class WpProQuiz_Model_QuestionMapper extends WpProQuiz_Model_Mapper
{
    ...
    public function save(WpProQuiz_Model_Question $question, $auto = false)
    {
        ...
        if ($auto && $question->getId() && !$this->checkThisThatQuiz($question->getQuizId())) {
            // This condition shouldn't execute if quiz is This or That
    ...
    public function fetchAll($quizId, $rand = false, $max = 0)
    {
        ...
        $a = $this->fetchQuestions($quizId,$results);
    ...
    public function fetchTable($quizId, $orderBy, $order, $search, $limit, $offset, $filter)
    {
        ...
        $r = $this->fetchQuestions($quizId,$results);
    ...
    /**
     * Common function to convert questions to the question object and also count the votes for this or that quiz
     * @param $quizId
     * @param array $results
     *
     * @return WpProQuiz_Model_Question[]
     */
    public function fetchQuestions($quizId,$results)
    {
        $thisThatStatus = $this->checkThisThatQuiz($quizId);
        ...
        if($thisThatStatus){
            ...
            $questionId = rtrim($questionId,',');

            if(!empty($questionId)){
                $countVotes = $this->_wpdb->get_results(
                    "SELECT question_id,group_concat(answer_data) as ans_data
                        FROM
                ...
                foreach($countVotes as $cv){
                    $id = $cv['question_id'];
                    $a_count = 0; $b_count = 0;
                    $ansData = json_decode("[".$cv['ans_data']."]");
                    foreach ($ansData as $ad){
                        $a_count = $a_count + $ad[0];
                        $b_count = $b_count + $ad[1];
                    }
                    $data[$id] = array('A' =>$a_count,'B'=>$b_count);
                }
            }
        }
            ...
            if($thisThatStatus){
                ...
                if($data){
                    if(!empty($data[$row['id']])){
                        $a_count = $data[$row['id']]['A'];
                        $b_count = $data[$row['id']]['B'];
                    }
                }
                $row['thisThatVotes'] = json_encode(array('A' => $a_count, 'B' => $b_count));
            }
     ...
     *  return bool
    */
    public function checkThisThatQuiz($id)
    {
        if($id){
            $quizMapper = new WpProQuiz_Model_QuizMapper();
            $quiz = $quizMapper->fetch($id);

            return ($quiz->getCategoryValue() == THIS_OR_THAT_QUIZ_VALUE);
        }
    }
}
