...
class WpProQuiz_Model_FormMapper extends WpProQuiz_Model_Mapper
{
    ...
    public function ApproveFieldId($quizId)
    {
        $formId = $this->_wpdb->get_var(
            $this->_wpdb->prepare(
                "SELECT form_id FROM {$this->_tableForm} WHERE quiz_id = %d AND type =%d",
                $quizId, WpProQuiz_Model_Form::FORM_TYPE_APPROVE
            )
        );
        return !empty($formId) ? $formId : 0;
    }
}
