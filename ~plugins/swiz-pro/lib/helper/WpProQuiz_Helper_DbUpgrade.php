...
class WpProQuiz_Helper_DbUpgrade
{
    ...
    public function upgrade($version)
    {
        ...
        // if admin tried to reactive and no changes made since plugin is installed
        if ((int)$version === WpProQuiz_Helper_DbUpgrade::WPPROQUIZ_DB_VERSION) {
            return WpProQuiz_Helper_DbUpgrade::WPPROQUIZ_DB_VERSION;
        }

        // if plugin is activated first time that means $version is null
        if ($version === false || ((int)$version) > WpProQuiz_Helper_DbUpgrade::WPPROQUIZ_DB_VERSION) {
            $this->install(); // Default queries to be installed
            $version = WPPROQUIZ_DEFAULT_DB_VERSION;
        }

        $version = (int)$version;

        // if admin tried to reactive plugin and new queries written by developers 
        $this->dbUpgrade($version); // Custom queries to be executed
     ...
     * dbUpgrade
     *
     * Execute database upgrade functions
     *
     * @param   int $version
     * @return  void
     */
    private function dbUpgrade($version)
    {
    	if($version){
	        do {
    ...
    private function upgradeDbV25()
    {
        $this->_wpdb->query('
			ALTER TABLE  ' . $this->_wpdb->prefix . 'wp_pro_quiz_master
				ADD  `facebook_share` TINYINT( 1 ) NOT NULL AFTER  `hide_result_points` ;
		');

        $this->_wpdb->query('
			ALTER TABLE  ' . $this->_wpdb->prefix . 'wp_pro_quiz_master
				ADD  `twitter_share` TINYINT( 1 ) NOT NULL AFTER  `facebook_share` ;
		');

        $this->_wpdb->query('
			ALTER TABLE  ' . $this->_wpdb->prefix . 'wp_pro_quiz_master
				ADD  `email_share` TINYINT( 1 ) NOT NULL AFTER  `twitter_share` ;
		');

        $this->_wpdb->query('
			ALTER TABLE  ' . $this->_wpdb->prefix . 'wp_pro_quiz_master
				ADD  `linkedin_share` TINYINT( 1 ) NOT NULL AFTER  `email_share` ;
		');
        return 26;
    }

    private function upgradeDbV26()
    {
        $this->_wpdb->query('
			ALTER TABLE  ' . $this->_wpdb->prefix . 'wp_pro_quiz_master
				ADD  `quiz_style` VARCHAR( 200 ) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL AFTER  `result_text` ;
		');

        $this->_wpdb->query('
			ALTER TABLE  ' . $this->_wpdb->prefix . 'wp_pro_quiz_master
				ADD  `custom_quiz_box` VARCHAR( 400 ) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL AFTER  `quiz_style` ;
		');
        return 27;
    }

    private function upgradeDbV27()
    {
        $this->_wpdb->query('
			ALTER TABLE  ' . $this->_wpdb->prefix . 'wp_pro_quiz_master
				CHANGE  `custom_quiz_box` `custom_quiz_box` LONGTEXT CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL;
		');
        return 28;
    }

    private function upgradeDbV28()
    {
        $this->_wpdb->query('
			ALTER TABLE  ' . $this->_wpdb->prefix . 'wp_pro_quiz_master
        ...
        $this->_wpdb->query('
			ALTER TABLE  ' . $this->_wpdb->prefix . 'wp_pro_quiz_master
				ADD  `terms_conditions` LONGTEXT NULL;
		');

        $this->_wpdb->query('
			ALTER TABLE  ' . $this->_wpdb->prefix . 'wp_pro_quiz_master
				ADD  `contest_rules` LONGTEXT NULL;
		');
        return 29;
    }

    private function upgradeDbV29()
    {
        $this->_wpdb->query('
			ALTER TABLE  ' . $this->_wpdb->prefix . 'wp_pro_quiz_master
				ADD  `is_privacy_policy` TINYINT( 1 ) NOT NULL AFTER  `privacy_policy` ;
		');

        $this->_wpdb->query('
			ALTER TABLE  ' . $this->_wpdb->prefix . 'wp_pro_quiz_master
				ADD  `is_terms_conditions` TINYINT( 1 ) NOT NULL AFTER  `terms_conditions` ;
		');

        $this->_wpdb->query('
			ALTER TABLE  ' . $this->_wpdb->prefix . 'wp_pro_quiz_master
				ADD  `is_contest_rules` TINYINT( 1 ) NOT NULL AFTER  `contest_rules` ;
		');

        $this->_wpdb->query('
			ALTER TABLE  ' . $this->_wpdb->prefix . 'wp_pro_quiz_master
				ADD  `privacy_policy_text` varchar(32) NULL DEFAULT "Privacy Policy";
		');

        $this->_wpdb->query('
			ALTER TABLE  ' . $this->_wpdb->prefix . 'wp_pro_quiz_master
				ADD  `terms_conditions_text` varchar(32) NULL DEFAULT "Terms &amp; Conditions";
		');

        $this->_wpdb->query('
			ALTER TABLE  ' . $this->_wpdb->prefix . 'wp_pro_quiz_master
				ADD  `contest_rules_text` varchar(32) NULL DEFAULT "Contest Rules";
		');

		return 30;
    }

    private function upgradeDbV30()
    {
        $this->_wpdb->query('
			ALTER TABLE  ' . $this->_wpdb->prefix . 'wp_pro_quiz_category
				ADD  `category_value` varchar(200) NULL COMMENT "Stores value which will use to compare on frontside" AFTER  `category_name`
		');

		return 31;
    }

    private function upgradeDbV31()
    {
        $this->_wpdb->query('
			INSERT INTO `' . $this->_wpdb->prefix . 'wp_pro_quiz_category` (category_name, category_value, type) VALUES ("'.GIVEAWAY_CONTEST_TEXT.'","'.GIVEAWAY_CONTEST_VALUE.'","quiz")
		');

        $this->_wpdb->query('
			INSERT INTO `' . $this->_wpdb->prefix . 'wp_pro_quiz_category` (category_name, category_value,type) VALUES ("'.PHOTO_CONTEST_TEXT.'","'.PHOTO_CONTEST_VALUE.'","quiz")
		');

        $this->_wpdb->query('
			INSERT INTO `' . $this->_wpdb->prefix . 'wp_pro_quiz_category` (category_name, category_value,type) VALUES ("'.CHALLENGE_QUIZ_TEXT.'","'.CHALLENGE_QUIZ_VALUE.'","quiz")
		');

        $this->_wpdb->query('
			INSERT INTO `' . $this->_wpdb->prefix . 'wp_pro_quiz_category` (category_name, category_value,type) VALUES ("'.THIS_OR_THAT_QUIZ_TEXT.'","'.THIS_OR_THAT_QUIZ_VALUE.'","quiz")
		');

        $this->_wpdb->query('
			INSERT INTO `' . $this->_wpdb->prefix . 'wp_pro_quiz_category` (category_name, category_value,type) VALUES ("'.BUCKETLIST_TEXT.'","'.BUCKETLIST_VALUE.'","quiz")
		');

		return 32;
    }

    private function upgradeDbV32()
    {
        $this->_wpdb->query('
			ALTER TABLE  ' . $this->_wpdb->prefix . 'wp_pro_quiz_master
				ADD  `custom_form_header` varchar(64) NULL DEFAULT "Information";
		');

		return 33;
    }

    private function upgradeDbV33()
    {
        $this->_wpdb->query('
			ALTER TABLE  ' . $this->_wpdb->prefix . 'wp_pro_quiz_master
				CHANGE  `custom_form_header` `custom_form_header` LONGTEXT NULL;
		');

        $this->_wpdb->query('
			ALTER TABLE  ' . $this->_wpdb->prefix . 'wp_pro_quiz_master
				ADD  `is_custom_form_header` TINYINT( 1 ) NOT NULL ;
		');

		return 34;
    }

    private function upgradeDbV34()
    {
        $this->_wpdb->query('
			ALTER TABLE  ' . $this->_wpdb->prefix . 'wp_pro_quiz_master
				ADD  `embed_code` VARCHAR(512) ;
		');
        return 35;
    }

    private function upgradeDbV35()
    {
        $this->_wpdb->query('
			ALTER TABLE  ' . $this->_wpdb->prefix . 'wp_pro_quiz_master
				ADD  `pc_left_header` varchar(64) NULL;
		');

        $this->_wpdb->query('
			ALTER TABLE  ' . $this->_wpdb->prefix . 'wp_pro_quiz_master
				ADD  `pc_center_header` varchar(64) NULL;
		');

        $this->_wpdb->query('
			ALTER TABLE  ' . $this->_wpdb->prefix . 'wp_pro_quiz_master
				ADD  `pc_right_header` varchar(64) NULL;
		');

        $this->_wpdb->query('
			ALTER TABLE  ' . $this->_wpdb->prefix . 'wp_pro_quiz_master
				ADD  `pc_description` LONGTEXT NULL;
		');

        return 36;
    }

    private function upgradeDbV36()
    {
        $this->_wpdb->query('
			ALTER TABLE  ' . $this->_wpdb->prefix . 'wp_pro_quiz_master
				ADD  `pc_left_header_title` varchar(32) NULL DEFAULT "' . DEFAULT_PC_LEFT_HEADER_TITLE . '";
		');

        $this->_wpdb->query('
			ALTER TABLE  ' . $this->_wpdb->prefix . 'wp_pro_quiz_master
				ADD  `pc_center_header_title` varchar(32) NULL DEFAULT "' . DEFAULT_PC_CENTER_HEADER_TITLE . '";
		');

        $this->_wpdb->query('
			ALTER TABLE  ' . $this->_wpdb->prefix . 'wp_pro_quiz_master
				ADD  `pc_right_header_title` varchar(32) NULL DEFAULT "' . DEFAULT_PC_RIGHT_HEADER_TITLE . '";
		');

		return 37;
    }

    private function upgradeDbV37()
    {
        $this->_wpdb->query('
			ALTER TABLE  ' . $this->_wpdb->prefix . 'wp_pro_quiz_master
				ADD  `quiz_sharing_description` varchar(128) NULL;
		');
