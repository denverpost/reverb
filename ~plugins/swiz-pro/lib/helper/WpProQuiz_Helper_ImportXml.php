...
class WpProQuiz_Helper_ImportXml
{
    ...
    private function createQuizModel($xml)
    {
        ...
        $model->setQuizStyle($xml->quizStyle);
        ...
        $model->setFacebookShare($xml->facebookShare == 'true');
        $model->setTwitterShare($xml->twitterShare == 'true');
        $model->setLinkedinShare($xml->linkedinShare == 'true');
        $model->setEmailShare($xml->emailShare == 'true');

        $model->setIsPrivacyPolicy($xml->isPrivacyPolicy == 'true');
        $model->setIsTermsConditions($xml->isTermsConditions == 'true');
        $model->setIsContestRules($xml->isContestRules == 'true');
        $model->setPrivacyPolicy($xml->privacyPolicy);
        $model->setTermsConditions($xml->termsConditions);
        $model->setContestRules($xml->contestRules);
        $model->setPrivacyPolicyText($xml->privacyPolicyText);
        $model->setTermsConditionsText($xml->termsConditionsText);
        $model->setContestRulesText($xml->contestRulesText);

        $arrDefaultStyles = array(SMALL_CHECKBOX_CSS,MEDIUM_CHECKBOX_CSS,LARGE_CHECKBOX_CSS,DENVERPOST_CSS,THEKNOW_CSS);
        if(!in_array($xml->quizStyle, $arrDefaultStyles)){
            //Quiz style is not among the default stylesheet so creating new stylesheet at custom style path.

            $fp = fopen(WPPROQUIZ_CUSTOM_STYLE_PATH .'/'. $xml->quizStyle,"wb");
            if($fp){
                fwrite($fp,$xml->customQuizBox);
                fclose($fp);
            }else{
                WpProQuiz_View_View::admin_notices(__('Failed to create stylesheet on server, please check permissions.', 'wp-pro-quiz'), 'error');
            }
        }

        $model->setQuizStyle($xml->quizStyle);
        $model->setCustomQuizBox($xml->customQuizBox);
        $model->setCategoryValue($xml->categoryValue);
        $model->setCustomFormHeader($xml->customFormHeader);
        $model->setIsCustomFormHeader($xml->isCustomFormHeader);
        $model->setEmbedCode($xml->embedCode);
        $model->setPcLeftHeader($xml->pcLeftHeader);
        $model->setPcCenterHeader($xml->pcCenterHeader);
        $model->setPcRightHeader($xml->pcRightHeader);
        $model->setPcLeftHeaderTitle($xml->pcLeftHeaderTitle);
        $model->setPcCenterHeaderTitle($xml->pcCenterHeaderTitle);
        $model->setPcRightHeaderTitle($xml->pcRightHeaderTitle);
        $model->setPcDescription($xml->pcDescription);
        $model->setQuizSharingDescription($xml->quizSharingDescription);
