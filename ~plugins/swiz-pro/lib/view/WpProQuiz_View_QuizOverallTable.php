...
class WpProQuiz_View_QuizOverallTable extends WP_List_Table
{
    ...
    public static function getColumnDefs()
    {
        $columns = array(
            ...
            'category' => __('Quiz Type', 'wp-pro-quiz'),
            'shortcode' => __('Shortcode', 'wp-pro-quiz'),
            'shortcode_leaderboard' => __('Embed Code', 'wp-pro-quiz')
        );
    ...
    {
        if($item['category'] != PHOTO_CONTEST_TEXT && $item['category'] !=GIVEAWAY_CONTEST_TEXT){
            $actions = array(
        ...
        */
        ...
        return sprintf('<a class="row-title" href="?page=wpProQuiz&action=addEdit&quizId=%1$s">%2$s</a> %3$s',
    ...
    function prepare_items()
    {
        ...
        foreach ($this->quizItems as $q) {
            $items[] = array(
                ...
                'shortcode_leaderboard' => "<div class='frameWrapper'>".esc_html($q->getEmbedCode())."</div>"
            );
        }
