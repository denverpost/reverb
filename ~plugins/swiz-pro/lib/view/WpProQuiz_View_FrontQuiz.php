...
class WpProQuiz_View_FrontQuiz extends WpProQuiz_View_View
{
    ...
    private function loadButtonNames()
    {
        ...
        $names['finish_quiz'] = $this->quiz->getCategoryValue() == BUCKETLIST_VALUE ? __('SEE WHERE YOU STAND', 'wp-pro-quiz') : (($this->quiz->getCategoryValue() == PHOTO_CONTEST_VALUE || $this->quiz->getCategoryValue() == GIVEAWAY_CONTEST_VALUE )? __('Enter', 'wp-pro-quiz') : $names['finish_quiz']);

        $names['start_quiz'] = ($this->quiz->getCategoryValue() == PHOTO_CONTEST_VALUE ||$this->quiz->getCategoryValue() == GIVEAWAY_CONTEST_VALUE)? __('Enter Contest', 'wp-pro-quiz') : $names['start_quiz'];
    ...
    public function show($preview = false)
    {
        ...
        ?>
        <div id="swizpro">
            <div class="wpProQuiz_content swizpro_quizWrapper" id="wpProQuiz_<?php echo $this->quiz->getId(); ?>">
                <?php

                echo '<div id="getQuizTitle" style="display:none;">'. $this->quiz->getName(). '</div>';

                if (!$this->quiz->isTitleHidden() && !$this->checkPhotoOrGiveAwayQuiz()) {
                  echo '
                  <div class ="quizHeader"><h1 class = "quizTitle">', $this->quiz->getName(), '</h1></div>';
                }
        ...
        <div class="wpProQuiz_forms">
            <ul>
                ...
                foreach ($this->forms as $form) {
                    ...
                    ?>
                    <li class="clearfix">
                        <?php if($form->getType()!=WpProQuiz_Model_Form::FORM_TYPE_APPROVE) {?>
                        <div class="fieldName fieldForm">
                            <?php
                            echo '<label for="' . $id . '">';
                        ...
                        <div class="fieldWrapp fieldForm">
                            <?php
                            switch ($form->getType()) {
                                ...
                                case WpProQuiz_Model_Form::FORM_TYPE_CHECKBOX:
                                    echo '<label class="customCheckWrap">';
                                    echo '<input name="' . $name . '" id="' . $id . '" type="checkbox" value="1"',
                                        'data-required="' . (int)$form->isRequired() . '" data-type="' . $form->getType() . '" data-form_id="' . $form->getFormId() . '" class="wpProQuiz_questionInput">';
                                    echo '<span class="checkmarkWrap"></span>';
                                    echo '</label>';
                                    break;
                                case WpProQuiz_Model_Form::FORM_TYPE_DATE:
                                    ...
                                    echo '</div>';
                                    break;
                                case WpProQuiz_Model_Form::FORM_TYPE_FILE:
                                    echo '<input name="' . $name . '" id="' . $id . '" type="file" ',
                                        'data-required="' . (int)$form->isRequired() . '" data-type="' . $form->getType() . '" data-form_id="' . $form->getFormId() . '">';

                                    break;
                                case WpProQuiz_Model_Form::FORM_TYPE_APPROVE:
                                    echo '<input name="approve_status" id="' . $id . '" type="hidden" ',
                                        'data-required="' . (int)$form->isRequired() . '" data-type="10" data-form_id="' . $form->getFormId() . '">';
                                    break;
                                case WpProQuiz_Model_Form::FORM_TYPE_RADIO:
                                    ...
                                    if ($form->getData() !== null) {
                                        foreach ($form->getData() as $data) {
                                            echo '<label>';
                                            echo '<input name="' . $name . '" type="radio" class="wpProQuiz_questionInput" value="' . esc_attr($data) . '"> ',
                                            esc_html($data);
                                            echo '</label> ';
                                        }
                                    }
                                ...
                                case WpProQuiz_Model_Form::FORM_TYPE_YES_NO:
                                    echo '<div data-required="' . (int)$form->isRequired() . '" data-type="' . $form->getType() . '" class="wpProQuiz_formFields" data-form_id="' . $form->getFormId() . '">';
                                    echo '<label class="radioInputWrap">';
                                    echo '<input name="' . $name . '" type="radio" value="1" class="wpProQuiz_questionInput"> ',
                                    __('Yes', 'wp-pro-quiz');
                                    echo '</label> ';

                                    echo '<label class="radioInputWrap">';
                                    echo '<input name="' . $name . '" type="radio" value="0" class="wpProQuiz_questionInput"> ',
                                    __('No', 'wp-pro-quiz');
                                    echo '</label> ';
                                    echo '</div>';
                                    break;
    ...
    private function showCheckPageBox($questionCount)
    {
        ...
        <div class="wpProQuiz_checkPage" style="display: none;">
            ...
            if ($this->quiz->isFormActivated() && $this->quiz->getFormShowPosition() == WpProQuiz_Model_Quiz::QUIZ_FORM_POSITION_END
                ...
                ?>
                <h4 class="wpProQuiz_header"><?php _e($this->quiz->isCustomFormHeader() ? $this->quiz->getCustomFormHeader() : DEFAULT_CUSTOM_FORM_HEADER, 'wp-pro-quiz'); ?></h4>
                <?php
                $this->showFormBox();
            }
    ...
    private function showInfoPageBox()
    {
        ...
        <div class="wpProQuiz_infopage" style="display: none;">
            <?php _e($this->quiz->isCustomFormHeader() ? $this->quiz->getCustomFormHeader() : DEFAULT_CUSTOM_FORM_HEADER, 'wp-pro-quiz'); ?>
    ...
    private function showStartQuizBox()
    {
        ...
        <div class="wpProQuiz_text">
            ...
            <?php if($this->checkPhotoOrGiveAwayQuiz()){ ?>
            <div class="middleDetailWrapper">
              <div class="contestWrapper">
                <ul class="clearfix">
                  <li>
                    <span class="detailContainer"><?php echo $this->quiz->getPcLeftHeaderTitle(); ?></span>
                    <span class="showDetail"><?php echo $this->quiz->getPcLeftHeader(); ?></span>
                  </li>
                  <li>
                    <span class="detailContainer"><?php echo $this->quiz->getPcCenterHeaderTitle(); ?></span>
                    <span class="showDetail"><?php echo $this->quiz->getPcCenterHeader(); ?></span>
                  </li>
                  <li>
                    <span class="detailContainer"><?php echo $this->quiz->getPcRightHeaderTitle(); ?></span>
                    <span class="showDetail"><?php echo $this->quiz->getPcRightHeader(); ?></span>
                  </li>
                </ul>
              </div>
            </div>
            <div class="bottomBannerWrapper">
                <p>
                    <?php echo do_shortcode(apply_filters('comment_text', $this->quiz->getPcDescription())); ?>
                </p>
            </div>
          <?php } 
            if (!$this->checkPhotoOrGiveAwayQuiz() && $this->quiz->isFormActivated() && $this->quiz->getFormShowPosition() == WpProQuiz_Model_Quiz::QUIZ_FORM_POSITION_START) {
                $this->showFormBox();
            }
            ?>
    ...
    private function viewPcGallery()
    {
        if($this->checkPhotoOrGiveAwayQuiz()){ ?>
            <div class="viewGalleryWrapper">
                <div class="contestWrapper">
                    <h3>View the Gallery</h3>
                    <div class="viewGallery">
                        <ul class="clearfix">
                        <?php foreach($this->pcImages as $pcImg){ echo $pcImg; } ?>
                        </ul>
                    </div>               
                    <?php if($this->totalPcRows > DEFAULT_PC_GALLERY_IMG){ ?>
                        <a href="javascript:void(0);" class="loadMoreButton">Load More</a>
                        <span id="startPage" style="display: none;"><?php echo DEFAULT_PC_GALLERY_IMG; ?></span>
...
<!--            <h4 class="wpProQuiz_header">--><?php //_e('Results', 'wp-pro-quiz'); ?><!--</h4>-->
            <?php if (!$this->quiz->isHideResultCorrectQuestion()) { ?>
                <p>
                    <?php printf(__('%s of %s questions answered correctly', 'wp-pro-quiz'),
                        '<span class="wpProQuiz_correct_answer">0</span>', '<span class="wpProQuiz_total_question">' . $questionCount . '</span>'); ?>
                </p>
            <?php }
            ...
            <div style="margin: 10px 0px; text-align: center;">
                <?php if (!$this->quiz->isBtnRestartQuizHidden()) { ?>
                    <input class="wpProQuiz_button restartQuizButton" type="button" name="restartQuiz"
                           value="<?php echo $this->_buttonNames['restart_quiz']; ?>">
                <?php }
                if (!$this->quiz->isBtnViewQuestionHidden()) { ?>
                    <input class="wpProQuiz_button" type="button" name="reShowQuestion"
                           value="<?php _e('View Answers', 'wp-pro-quiz'); ?>">
                <?php } ?>
            ...
            <?php
            if($this->quiz->isFacebookShare() || $this->quiz->isTwitterShare() || $this->quiz->isLinkedinShare() || $this->quiz->isEmailShare()) { ?>
            <div class="share-control">
                <span class="titleShare"> <?php echo $this->checkPhotoOrGiveAwayQuiz() ? "Share this Contest" : "Share your results"; ?></span>
                <div>
                    <?php
                    if($this->quiz->isEmailShare()){ ?>
                        <a href="javascript:void(0);" class="emailSocial" id='emailShare'>
                            <i class='iconMain iconMain-email'></i>
                            <span class="socialTitle">EMAIL</span>
                            <i class="fa fa-envelope" aria-hidden="true"></i>
                        </a>
                    <?php }
                    if($this->quiz->isFacebookShare()){ ?>
                        <a href="javascript:void(0);" class="fbShare" id='fbShare'>
                            <i class='iconMain iconMain-facebook'></i>
                            <span class="socialTitle">SHARE</span>
                            <i class="fa fa-facebook" aria-hidden="true"></i>
                        </a>
                    <?php }
                    if($this->quiz->isTwitterShare()){ ?>
                        <a href="javascript:void(0);" class="twitterShare" id='twitterShare'>
                            <i class='iconMain iconMain-twitter'></i>
                            <span class="socialTitle">TWEET</span>
                            <i class="fa fa-twitter" aria-hidden="true"></i>
                        </a>
                    <?php }
                    if($this->quiz->isLinkedinShare()){ ?>
                        <a href="javascript:void(0);" class="linkedinShare" id='linkedinShare'>
                            <i class='iconMain iconMain-linkedin'></i>
    ...
    private function showQuizBox($questionCount)
    {
        ...
        $json = array();
        $catPoints = array();
        $thisThatClass = $this->checkThisThatQuiz() ? THIS_THAT_CLASS : '';
        ?>

        <div style="display: none;" class="wpProQuiz_quiz <?php echo $thisThatClass; ?>">
            <ol class="wpProQuiz_list">
                ...
                foreach ($this->question as $question) {
                    ...
                    $questionClass = ($question->getAnswerType() == 'free_answer') ? FREECHOICE_QUESTION_CLASS : '';

                    $questionText = do_shortcode(apply_filters('comment_text', $question->getQuestion()));
                    $questionImgClass = "";
                    if (strpos($questionText, 'img') !== false) {
                        $questionImgClass = "questionWithImg";
                    }

                    ?>
                    <li class="wpProQuiz_listItem <?php echo $questionClass.' '.$questionImgClass; ?>" style="display: none;">
                        <div
                        ...
                        <h5 style="<?php echo $this->quiz->isHideQuestionNumbering() ? 'display: none;' : 'display: inline-block;' ?>"
                            ...
                            <?php _e('Question', 'wp-pro-quiz'); ?> <span><?php echo $index; ?></span>
                        </h5>

                        <?php if ($this->quiz->isShowPoints()) { 
                                $pointsText = $question->getPoints() > 1 ? "points" : "point";
                        ?>
                            <span style="font-weight: bold; float: right;"><?php printf(__('%d '.$pointsText, 'wp-pro-quiz'),
                        ...
                        <div class="wpProQuiz_question" style="margin: 10px 0 0 0;">
                            ...
                            <?php } ?>
                            <ul class="wpProQuiz_questionList clearfix" data-question_id="<?php echo $question->getId(); ?>"
                                data-type="<?php echo $question->getAnswerType(); ?>">
                                <?php
                                $answer_index = 0;
                                $quesionAnswerCount = 0;

                                foreach ($answerArray as $v) {
                                    $countVotes = json_decode($question->getThisThatVotes(),true);
                                    ...
                                    if ($answer_text == '') {
                                      ...
                                      $imgClass = "";
                                    if (strpos($answer_text, 'img') !== false) {
                                        $imgClass = "answerImage";
                                    }
                                    ...
                                    ?>
                                    <li class="wpProQuiz_questionListItem <?php echo $imgClass; ?>" data-pos="<?php echo $answer_index; ?>">
                                        <?php if ($question->getAnswerType() === 'single' || $question->getAnswerType() === 'multiple') {  ?>
                                            ...
                                            <label class="container">
                                                <input class="wpProQuiz_questionInput"
                                                       ...
                                                       value="<?php echo($answer_index + 1); ?>"><span class="answer_text"><?php echo $answer_text; ?></span>
                                                       <span class="checkmark"></span>
                                            </label>
                                            <div class="resultUpDownVote" style="display: none;">
                                            <span class="resultVotes">
                                            <?php
                                                if($answer_index == 0){
                                                    echo isset($countVotes['A']) ? $countVotes['A'] : 0;
                                                }
                                                if($answer_index == 1){
                                                    echo isset($countVotes['B']) ? $countVotes['B'] : 0;
                                                }
                                            ?>
                                            </span>
                                            <span class="resultVoteText">votes</span>
                                            </div>
                                ...
                                echo $this->checkThisThatQuiz() ? '<span class="youVoted userVote" style="display: none"></span>' : "";
                                ?>
                            </ul>
                            
                        </div>
                        <?php if (!$this->quiz->isHideAnswerMessageBox()) { ?>
                            <div class="wpProQuiz_response clearfix" style="display: none;">
                                <div style="display: none;" class="wpProQuiz_correct">
                                    <?php if ($question->isShowPointsInBox() && $question->isAnswerPointsActivated()) { ?>
                                        <div>
                                    <span class="wpProQuiz_respone_span leftWrap">
                                        <?php _e('Correct', 'wp-pro-quiz'); ?>
                                    </span>
                                    <span class="rightWrap"><?php echo $question->getPoints() . ' / ' . $question->getPoints(); ?><?php _e('Points', 'wp-pro-quiz'); ?></span>
                                </div>
                                    <?php } else { ?>
        ...
        <div class="footerLinks">
            <?php if($this->quiz->isPrivacyPolicy()){ ?>
            <span id="privacy_policy"><?php echo $this->quiz->getPrivacyPolicyText(); ?></span>
            <div class="dialog_privacy_policy" title="<?php echo $this->quiz->getPrivacyPolicyText(); ?>" style="display:none"><?php echo $this->footerShortCode($this->quiz->getPrivacyPolicy()); ?></div>
            <?php } ?>

            <?php if($this->quiz->isTermsConditions()){ ?>
            <span id="terms_condition"><?php echo $this->quiz->getTermsConditionsText(); ?></span>
            <div class="dialog_terms_condition" title="<?php echo $this->quiz->getTermsConditionsText(); ?>" style="display:none"><?php echo $this->footerShortCode($this->quiz->getTermsConditions()); ?></div>
            <?php } ?>

            <?php if($this->quiz->isContestRules()){ ?>
            <span id="contest_rules"><?php echo $this->quiz->getContestRulesText(); ?></span>
            <div class="dialog_contest_rules" title="<?php echo $this->quiz->getContestRulesText(); ?>" style="display:none"><?php echo $this->footerShortCode($this->quiz->getContestRules()); ?></div>
            <?php } ?>        </div>
        <?php
    ...
    /**
     * Returns the content with applied filter to helps to display footer data in proper format
     * @param string $strContent
     * @return string
     */
    private function footerShortCode($strContent)
    {
        return do_shortcode(apply_filters('comment_text', $strContent));
    }
