...
class WpProQuiz_View_QuestionOverallTable extends WP_List_Table
{
    ...
    public static function getColumnDefs()
    {
        ...
        $quizWrapper = new WpProQuiz_Model_QuizMapper();
        $quiz = $quizWrapper->fetch($quizId);

        $thisThatStatus = $quiz->getCategoryValue() == THIS_OR_THAT_QUIZ_VALUE;
        ...
        if($thisThatStatus){
            $columns['ansA'] = __('Option A', 'wp-pro-quiz');
            $columns['ansB'] = __('Option B', 'wp-pro-quiz');
            $columns['ansTotal'] = __('Total Votes', 'wp-pro-quiz');
        }else{
            $columns['points'] = __('Points', 'wp-pro-quiz');
        }
    ...
    function prepare_items()
    {
        ...
        foreach ($this->questionItems as $i => $q) {
            $countVotes = json_decode($q->getThisThatVotes(),true);
            ...
            $items[] = array(
                ...
                'ansA' => $countVotes['A'],
                'ansB' => $countVotes['B'],
                'ansTotal' => $countVotes['A'] + $countVotes['B'],
            );
        }
