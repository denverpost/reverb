...
class WpProQuiz_Controller_Question extends WpProQuiz_Controller_Controller
{
    ...
    public function route()
    {
        ...
        $quiz = $m->fetch($this->_quizId);
        //if quiz type is photo contest or giveaway then return null
        if($quiz->getCategoryValue() == PHOTO_CONTEST_VALUE || $quiz->getCategoryValue() == GIVEAWAY_CONTEST_VALUE) return;
    ...
    private function getPostQuestionModel($quizId, $questionId)
    {
        ...
        //Set it to zero because it will never receive category from admin as category accordian is removed
        $post['categoryId'] = 0;
