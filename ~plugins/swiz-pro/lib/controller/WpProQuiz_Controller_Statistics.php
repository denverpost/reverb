...
class WpProQuiz_Controller_Statistics extends WpProQuiz_Controller_Controller
{
    ...
    private function makeFormData($quiz, $data)
    {
        ...
        foreach ($forms as $form) {
            if ($form->getType() != WpProQuiz_Model_Form::FORM_TYPE_DATE || $form->getType() == WpProQuiz_Model_Form::FORM_TYPE_EMAIL) {
                $str = isset($data[$form->getFormId()]) ? $data[$form->getFormId()] : '';

                if($form->getType() == WpProQuiz_Model_Form::FORM_TYPE_EMAIL){
                    $str = strtolower($str);
                }
    ...
    public static function ajaxLoadHistory($data,$returnJson = true)
    {
        if (!current_user_can('wpProQuiz_show_statistics')) {
            return $returnJson ? json_encode(array()) : array();
        }
        ...
        foreach ($statisticModel as $model) {
            ...
            if($quiz->getCategoryValue() == PHOTO_CONTEST_VALUE || $quiz->getCategoryValue() == GIVEAWAY_CONTEST_VALUE ){
                $result = '0%';
                $formatCorrect = '0 (0%)';
                $formatInCorrect = '0 (0%)';
            }else{
                $result = round(100 * $model->getPoints() / $model->getGPoints(), 2) . '%';
                $formatCorrect = $model->getCorrectCount() . ' (' . round(100 * $model->getCorrectCount() / $sum,
                    2) . '%)';
                $formatInCorrect = $model->getIncorrectCount() . ' (' . round(100 * $model->getIncorrectCount() / $sum,
                    2) . '%)';
            }
            ...
            $model->setFormatCorrect($formatCorrect);
            $model->setFormatIncorrect($formatInCorrect);
        ...
        $arrResponse = array('html' => $html,'navi' => $navi);

        return ($returnJson) ? json_encode($arrResponse) : $arrResponse;
    }
    ...
    public static function ajaxRestApproveStatistic($data)
    {
        ...
        $statisticRefMapper = new WpProQuiz_Model_StatisticRefMapper();
        $statisticRefMapper->approveByRefId($data['refId'],$data['quizId']);
