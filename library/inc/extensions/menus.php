...
function reactor_register_menus() {
	...
	if ( in_array( 'top-bar-outdoors', $menus[0] ) ) {
		register_nav_menu('top-bar-outdoors', __( 'Outdoors', 'reactor'));
	}
...
/**
 * Top bar Outdoors menu
 *
 * @since 1.0.0
 * @see wp_nav_menu
 * @param array $locations Associative array of menu location identifiers (like a slug) and descriptive text.
 */
if ( !function_exists('reactor_top_bar_outdoors') ) { 
	function reactor_top_bar_outdoors() {
		$defaults = array( 
			'theme_location'  => 'top-bar-outdoors',
			'container'       => false,
			'menu_class'      => 'top-bar-menu left',
			'echo'            => 0,
			'fallback_cb'     => false,
