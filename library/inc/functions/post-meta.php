			  ...
			  $bio_author_meta = array_map( function( $a ){ return $a[0]; }, get_user_meta( $bio_author->ID ) );
				...
				if( $args['show_photo'] ) {
					$author_social = sprintf('<ul class="author-social-small right inline-list">%1$s%2$s%3$s%4$s%5$s</ul>',
			            ( isset( $bio_author_meta['twitter'] ) && $bio_author_meta['twitter'] != '' ? sprintf('<li><a href="https://twitter.com/%1$s" title="%1$s on Twitter">Twitter</a></li>', $bio_author_meta['twitter'] ) : '' ),
			            ...
			            ( isset( $bio_author_meta['instagram'] ) && $bio_author_meta['instagram'] != '' ? sprintf('<li><a href="https://instagram.com/%1$s" title="%1$s on Instagram">Instagram</a></li>', $bio_author_meta['instagram'] ) : '' ),
